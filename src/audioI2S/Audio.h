...
class Audio {
  ...
  public:
    ...
    // FusionEdge: SPI DMA lock/unlock callback a reconfigI2S() körüli display-védelemhez
    inline static std::function<void(bool lock)> spi_lock_callback;
    ...
    bool             hasUnsupportedContent() { return m_f_unsupportedContent; } // FusionEdge: true if connect ok but stream content was not playable
    void             loop();
    uint32_t         stopSong();
  ...
  public:
    ...
    // --- FusionEdge BT bridge: thin public wrappers around the private
    //     I2S TX channel, used by bluetooth.cpp to feed externally
    //     received (BT) PCM straight into the same DAC output channel
    //     used for normal radio/SD playback. Do not call while the
    //     normal audioTask is actively decoding/writing. ---
    bool i2sEnableExt() { return I2Sstart() == ESP_OK; }
    bool i2sDisableExt() { return I2Sstop() == ESP_OK; }
    bool i2sWriteExt(const uint8_t* data, size_t len, size_t* bytesWritten, uint32_t timeoutMs = 20) {
        if (!m_i2s_tx_handle) return false;
        return i2s_channel_write(m_i2s_tx_handle, data, len, bytesWritten, pdMS_TO_TICKS(timeoutMs)) == ESP_OK;
    }
...
#pragma GCC diagnostic pop
    ...
    uint8_t        m_audioTaskCoreId = 1;           // FusionEdge: core 1 (WiFi/TCP stack tehermentesítése)
    ...
    bool           m_f_unsupportedContent = false;  // connect succeeded but response had no playable codec/playlist (FusionEdge)
