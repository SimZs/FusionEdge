...
// FusionEdge megjegyzés: a 18000-es érték (3.4.6q) "wrap holtsávot" okoz a
// readSpace()-ben – ha a read pointer a circular buffer végéhez <18000 byte-on
// belülre kerül, readSpace() < getMaxBlockSize() lesz akkor is, ha bufferFilled()
// bőven elég, és a decode periodikusan leáll → hallható "csettegés" AAC streamnél.
// 4608 = 3 * 1536 (AAC frame ~1536 minta/csatorna), elegendő headroom a régi
// 3200-hoz képest, de jóval kisebb a buffer-végi holtsávhoz vezető 18000-nél.
constexpr size_t m_frameSizeAAC = 4608;
...
constexpr size_t AUDIO_STACK_SIZE = 4500;
...
bool Audio::connecttohost(const char* host, const char* user, const char* pwd) { // user and pwd for authentification only, can be empty
    ...
    m_f_unsupportedContent = false; // FusionEdge: reset before each connect attempt
    ...
    if (m_f_ssl) {
        // FusionEdge: without this, NetworkClientSecure::connect() can block
        // indefinitely on some TLS1.3/HTTP2 (ALPN h2) servers - setTimeout()
        // above only governs socket-level reads, not the overall handshake.
        // Bound it so a stuck handshake fails fast instead of hanging the
        // player forever. setHandshakeTimeout() takes seconds in arduino-esp32;
        // round up m_timeout_ms_ssl (ms) to whole seconds, minimum 1.
        clientsecure.setHandshakeTimeout((m_timeout_ms_ssl + 999) / 1000);
    }
...
exit: // termination condition
    m_f_alt_user_agent = false;
    m_f_unsupportedContent = true; // FusionEdge: connection succeeded but response wasn't a playable stream/playlist
    m_dataMode = AUDIO_NONE;
    stopSong();
    return false;
...
void Audio::reconfigI2S() {
    ...
    // FusionEdge: SPI DMA védelme reconfigI2S körül (LovyanGFX ütközés elkerülése)
    if (spi_lock_callback) spi_lock_callback(true);
    ...
    if (spi_lock_callback) spi_lock_callback(false);
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
void Audio::startAudioTask() {
    ...
    m_audioTaskHandle = xTaskCreateStaticPinnedToCore(&Audio::audioTaskWrapper, /* Function to implement the task */
                                                      ...
                                                      4,                        /* Priority of the task - FusionEdge: 4 (high bitrate FLAC) */
