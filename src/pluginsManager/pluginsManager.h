...
class Plugin {
  public:
     ...
     * Location: MyNetwork::begin(), FusionEdge/src/core/network.cpp
     */
     ...
     * Location:
     * - Player::_play(uint16_t stationId), FusionEdge/src/core/player.cpp
     * - Player::browseUrl(), FusionEdge/src/core/player.cpp
     */
    virtual void on_start_play() __attribute__((weak)) {}

    /**
     * Triggered when playback stops.
     * Location: Player::_stop(bool alreadyStopped), FusionEdge/src/core/player.cpp
     */
    virtual void on_stop_play() __attribute__((weak)) {}

    /**
     * Triggered when the current track changes.
     * Location: Display::_title(), FusionEdge/src/core/display.cpp
     */
    virtual void on_track_change() __attribute__((weak)) {}

    /**
     * Triggered when the current station changes.
     * Location: Player::loop(), FusionEdge/src/core/player.cpp
     */
     ...
     * Parameters:
     * - request: Reference to a requestParams_t structure (defined in FusionEdge/src/core/common.h).
     * - result: Set to `false` to stop queue processing or `true` to continue.
     * Location: Display::loop(), FusionEdge/src/core/display.cpp
     */
     ...
     * Location:
     * - Display::_start(), FusionEdge/src/core/display.cpp
     * - Display::_swichMode(displayMode_e newmode), FusionEdge/src/core/display.cpp
     */
    virtual void on_display_player() __attribute__((weak)) {}

    /**
     * Triggered once every second.
     * Location: ticks(), FusionEdge/src/core/network.cpp
     */
     ...
     * Parameters:
     * - btnid: Reference to the button ID (defined in FusionEdge/src/core/common.h).
     * Location: onBtnClick(int id), FusionEdge/src/core/controls.cpp
     */
    virtual void on_btn_click(controlEvt_e& btnid) __attribute__((weak)) {}

    /**
     * Triggered on user interaction events (e.g. touchscreen tap release).
     * Parameters:
     * - consumed: Set to `true` to indicate interaction was handled by a plugin.
     */
    virtual void on_user_activity(bool& consumed) __attribute__((weak)) {}

    /**
     * Called every loop() iteration. Used for time-critical plugins like LED strip.
     * Location: loop(), FusionEdge.ino or display.cpp
     */
...
class pluginsManager {
  public:
    ...
    template <typename Func, typename... Args> inline void call_event(Func&& func, Args&&... args) {
        for (auto* plugin : plugins) {
            if (plugin) { (plugin->*func)(std::forward<Args>(args)...); }
        }
    }
    ...
    inline void on_user_activity(bool& consumed) { call_event(&Plugin::on_user_activity, consumed); }
    inline void on_loop() { call_event(&Plugin::on_loop); }

  private:
    std::vector<Plugin*> plugins;
    bool                 _ready = false;
    std::vector<Plugin*> pending;
};
