...
#    else
    ...
    if (useIndependentEncoders()) {
        if (first) {
            ...
            display.purgeQueuedRequestType(DRAWPLAYLIST);
...
void onBtnClick(int id) {
    ...
    switch (btnid) {
        ...
        case EVT_ENCBTNB:
        case EVT_ENC2BTNB: {
            // MODESELECT: egyszeri klikk megerősít
            if (display.mode() == MODESELECT) {
                display.modeSelectorConfirm();
                break;
            }
            if (display.mode() == NUMBERS) {
                display.numOfNextStation = 0;
...
void onBtnDoubleClick(int id) {
    ...
    switch ((controlEvt_e)id) {
        ...
        case EVT_ENCBTNB:
        case EVT_ENC2BTNB: {
            // MODESELECT módban: megerősítés
            if (display.mode() == MODESELECT) {
                display.modeSelectorConfirm();
                break;
            }
            // Dupla klikk PLAYER módban → mód-választó menü megnyitása
            if (display.mode() == PLAYER) {
