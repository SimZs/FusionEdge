...
namespace {
void lockDisplayForSd() {
#if !(DSP_MODEL == DSP_AXS15231B && SD_SPI_HOST == FSPI)
  display.lock();
  display.waitDMA();
#endif
}

void unlockDisplayForSd() {
#if !(DSP_MODEL == DSP_AXS15231B && SD_SPI_HOST == FSPI)
  display.unlock();
#endif
}
...
bool SDManager::start(){
  // The AXS15231B display uses SPI3 while its SD card uses SPI2/FSPI.
  // Other displays may share the SD bus, so they keep the display guard.
  lockDisplayForSd();
