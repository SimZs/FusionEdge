...
class DisplayMutexGuard {
  public:
    explicit DisplayMutexGuard(TickType_t ticks) {
        _taken = (displayMutex == nullptr) || (xSemaphoreTakeRecursive(displayMutex, ticks) == pdTRUE);
    }
    ~DisplayMutexGuard() {
...
#    endif
    if (newmode == STATIONS) {
        ...
        _plwidget->setDrawArea(VOL_AREA_TOP, VOL_AREA_HEIGHT);
