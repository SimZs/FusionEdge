...
class DspCore : public yoDisplay { // yoDisplay: typedef LGFX yoDisplay
  ...
  public:
    ...
    inline void drawLogo(uint16_t top) {
        ...
        // The 300x118 logo is nearly edge-to-edge on these 320px-wide panels
        // at native size; scale it down a bit for some breathing room.
        constexpr float LOGO_SCALE = 0.80f;
#else
        constexpr float LOGO_SCALE = 1.0f;
#endif
        drawPng(buf, size, 0, top, 0, 0, 0, 0, LOGO_SCALE, LOGO_SCALE, datum_t::top_center);
        free(buf);
    }
#endif
    void  flip();
