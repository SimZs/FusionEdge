...
class textBoxWidget : public Widget {
  public:
    ...
    textBoxWidget(textBoxConfig conf, uint16_t buffsize, bool uppercase, uint16_t fgcolor, uint16_t bgcolor, uint16_t borderColor, uint8_t fallbackTextSize = 2) {

        _fgcolor = fgcolor;
        _bgcolor = bgcolor;
        _borderColor = borderColor;
        _fallbackTextSize = fallbackTextSize;
  ...
  protected:
    ...
    uint8_t _fallbackTextSize = 2; // built-in font size used when the configured VLW font isn't available yet (e.g. before fonts are uploaded to LittleFS)
