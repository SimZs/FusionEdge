...
class WeatherIconWidget : public Widget {
  public:
    ...
    // Rendered box size (per-display). SRC_W/SRC_H is the native size the weather PNG
    // assets are authored at; ICO_W/ICO_H is the on-screen size, scaled to fit in
    // iconWidgets.cpp so the icon shrinks instead of getting cropped.
    static constexpr uint16_t SRC_W = 64;
    static constexpr uint16_t SRC_H = 64;
#if DSP_MODEL == DSP_ILI9341 || DSP_MODEL == DSP_ST7789
    static constexpr uint16_t ICO_W = 40;
    static constexpr uint16_t ICO_H = 40;
#else
...
class StationIconWidget : public Widget {
  public:
    ...
    // Rendered box size (per-display). SRC_W/SRC_H is the native size the station PNG
    // assets are authored at; ICO_W/ICO_H is the on-screen size, scaled to fit in
    // iconWidgets.cpp so the logo shrinks instead of getting cropped.
    static constexpr uint16_t SRC_W = 80;
    static constexpr uint16_t SRC_H = 80;
#if DSP_MODEL == DSP_ILI9341 || DSP_MODEL == DSP_ST7789
    static constexpr uint16_t ICO_W = 64;
    static constexpr uint16_t ICO_H = 64;
#else
