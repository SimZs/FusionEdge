...
#ifdef VU_DEBUG_PRINT
    // TEMPORARY verification tap for the stereo VU meter port -- confirms
    // player.getVULeft()/getVURight() move with real audio before any
    // rendering code is built. Remove this block (and the VU_DEBUG_PRINT
    // #define in myoptions.h) once the real widget is in place.
    {
        static uint32_t lastPrint = 0;
        uint32_t now = millis();
        if (now - lastPrint >= 500) {
            lastPrint = now;
            Serial.printf("[VU_DEBUG] L=%3u R=%3u\n", player.getVULeft(), player.getVURight());
        }
    }
#endif
