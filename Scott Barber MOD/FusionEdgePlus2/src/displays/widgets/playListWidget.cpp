...
void PlayListWidget::_prepareMovingModeLayout() {
    ...
    _plTtemsCount = _areaHeight / _plItemHeight;
    ...
    _plYStart = _areaTop + (_areaHeight - contentHeight) / 2;
    ...
    Serial.printf("PlayListWidget init (MOVING_CURSOR): areaTop=%d, areaH=%d, itemH=%d, items=%d, yStart=%d\n", _areaTop, _areaHeight, _plItemHeight, _plTtemsCount, _plYStart);
}
...
void PlayListWidget::_prepareFixedModeLayout() {
    ...
    uint8_t totalRows = _areaHeight / _plItemHeight;
...
void PlayListWidget::init(ScrollWidget* current) {
    ...
    // Ha setDrawArea nem lett hívva, az egész képernyő a terület
    if (_areaHeight == 0) { _areaTop = 0; _areaHeight = dsp.height(); }
...
void PlayListWidget::drawPlaylist(uint16_t currentItem) {
    ...
    if (movingMode) {
        ...
        if (pageChanged || isLongPause || _plCache[0].length() == 0) {
            ...
            // Teljes listaterület törlése (sávra korlátozva)
            dsp.fillRect(0, _areaTop, dsp.width(), _areaHeight, config.theme.background);
...
void PlayListWidget::_renderStaticList(uint16_t centerIdx) {
    ...
    // Háttér takarítása (sávra korlátozva)
    if ((int32_t)centralY > (int32_t)_areaTop)
        dsp.fillRect(0, _areaTop, dsp.width(), centralY - _areaTop, config.theme.background);
    uint32_t lowerY = centralY + _plItemHeight;
    uint32_t areaBottom = _areaTop + _areaHeight;
    if (lowerY < areaBottom)
        dsp.fillRect(0, lowerY, dsp.width(), areaBottom - lowerY, config.theme.background);
    ...
    for (int i = 1; i <= _sideRows; i++) {
        ...
        if (colorIdx >= 5) colorIdx = 4; // Max szín index
        dsp.setTextColor(config.theme.playlist[colorIdx], config.theme.background);
        if (y > 0 && y < (int)(_areaTop + _areaHeight)) dsp.drawString(_getCachedName(idx), 10, y);
    }
