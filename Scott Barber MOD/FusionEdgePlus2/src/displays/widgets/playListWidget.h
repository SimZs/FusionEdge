...
class PlayListWidget : public Widget {
  public:
    ...
    uint16_t currentTop() { return _areaTop + (_areaHeight / 2) - (_plItemHeight / 2); }
    void     setDrawArea(uint16_t top, uint16_t height) { _areaTop = top; _areaHeight = height; }

  private:
    ScrollWidget* _currentScroll = nullptr;
    uint16_t      _areaTop = 0;
    uint16_t      _areaHeight = 0;
