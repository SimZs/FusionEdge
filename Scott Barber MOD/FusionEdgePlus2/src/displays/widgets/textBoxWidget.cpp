...
bool textBoxWidget::_applyFont() {
    ...
    if (_boxconf.textsize == 9 && font_vlw_9) {
        ...
        ok = true;
    } else if (_boxconf.textsize == 11 && font_vlw_11) {
        _spr->loadFont(font_vlw_11);
        ok = true;
    ...
    _spr->setFont(nullptr);
    _spr->setTextSize(_fallbackTextSize);
    _loadedFontSize = _boxconf.textsize;
    _loadedAsVlw = false;
    return false;
}
...
void textBoxWidget::setText(const char* txt) {
    ...
    const bool stillOnFallback = (_boxconf.textsize >= 5) && !_loadedAsVlw;

    if (strcmp(_oldtext, _text) == 0 && !stillOnFallback) return;
