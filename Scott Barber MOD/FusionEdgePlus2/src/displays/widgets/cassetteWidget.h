...
class CassetteWidget : public Widget {
  ...
  private:
    ...
    float        _pngScale = 1.0f; // 1.0 = native 456x291 art; <1.0 = scaled down to fit a small screen
    char         _currentTapePath[64] = {};
    int16_t      _tapeReelLeftX = 131;  // native (456x291) reel-hole centers for the current tape
    int16_t      _tapeReelRightX = 327;
    int16_t      _tapeReelY = 129;
    ...
    void _selectTapeForStation(const char* station);
    void _drawBody();
    ...
    void _fitAudioInfo(const char* codec, const char* bitrateToken, const char* sampleToken,
                        const char* bitsToken, char* out, size_t outSize);
    void _drawReels();
