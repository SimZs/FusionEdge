...
// Writes FusionEdge format: title \t url \t ovol
// ================================================================
                                  ...
                                  uint32_t hardLimit)
{
  ...
  // ===== LittleFS sanity check =====
  File ftest = LittleFS.open(TMP_PATH, "w");
  if (!ftest) {
    Serial.println("[DLNA][PL] LittleFS open failed");
    return false;
  }
  ftest.close();
...
bool DlnaIndex::buildContainerIndex(const String& controlUrl, const String& rootObjectId) {
  ...
  File f = LittleFS.open(DLNA_LIST_JSON_PATH, "w");
  if (!f) return false;
