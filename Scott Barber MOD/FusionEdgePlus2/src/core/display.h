...
// Shared display-bus lock. display.cpp's own DisplayMutexGuard usages stay
// where they are; this is the same primitive, exposed so widget .cpp files
// outside display.cpp (cassetteWidget.cpp, coverArtWidget.cpp) can hold the
// bus across their own multi-step draw+push sequences. display.waitDMA()
// alone isn't enough for that: it takes the lock, waits for the current
// transfer, and releases it again *inside that one call* -- nothing stops
// another task from grabbing the bus in the gap between that wait and this
// widget's own later pushSprite(). Recursive, so it nests safely with
// anything else (including display.waitDMA() itself) that also takes it.
class DisplayMutexGuard {
  public:
    // ticksToWait mirrors FreeRTOS's TickType_t but is declared as uint32_t
    // here to avoid pulling FreeRTOS headers into this widely-included file.
    // Default (0xFFFFFFFFu) waits indefinitely, same value as portMAX_DELAY
    // on ESP32's 32-bit tick type.
    explicit DisplayMutexGuard(uint32_t ticksToWait = 0xFFFFFFFFu);
    ~DisplayMutexGuard();
...
#    ifdef USE_COVERART_SCREENSAVER
    CoverArtWidget* _coverartwidget = nullptr;
#    endif
