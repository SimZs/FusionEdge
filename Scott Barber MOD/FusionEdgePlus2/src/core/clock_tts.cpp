...
void clock_tts_loop() {
  ...
  if (clock_tts_fading_down) {
    ...
    if (clock_tts_fade_volume <= 0) {
      ...
      log_i("##[TTSDIAG]# speech begin time=%02d:%02d station=%d text='%s'",
            tm_struct->tm_hour, tm_struct->tm_min, clock_tts_saved_station, buf);
      player.setVolume(clock_tts_prev_volume);
      if (player.connecttospeech(buf, clock_tts_language)) {
        log_i("##[TTSDIAG]# speech connected station=%d", clock_tts_saved_station);
        ...
        log_w("##[TTSDIAG]# speech failed, resume station=%d", clock_tts_saved_station);
  ...
  if (clock_ttsActive) {
    if (clock_tts_resume_after
        && (config.getMode() != PM_WEB || config.lastStation() != clock_tts_saved_station)) {
      log_i("##[TTSDIAG]# user selection detected, cancel resume saved=%d current=%u mode=%u",
            clock_tts_saved_station,
            (unsigned)config.lastStation(),
            (unsigned)config.getMode());
      clock_tts_resume_after = false;
    }
    ...
    if (clock_tts_resume_after) {
      log_i("##[TTSDIAG]# speech finished, resume station=%d elapsed=%lu progress=%d",
            clock_tts_saved_station,
            (unsigned long)(nowMillis - clock_tts_started_at),
            clock_tts_audio_progress_seen ? 1 : 0);
      player.sendCommand({PR_PLAY, clock_tts_saved_station});
      clock_tts_fading_up = true;
      clock_tts_fade_timer = nowMillis;
    } else {
      log_i("##[TTSDIAG]# speech finished without stream resume elapsed=%lu progress=%d",
            (unsigned long)(nowMillis - clock_tts_started_at),
            clock_tts_audio_progress_seen ? 1 : 0);
    }
    clock_tts_started_at = 0;
