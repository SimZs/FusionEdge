...
portMUX_TYPE g_wsPlayMux = portMUX_INITIALIZER_UNLOCKED;
int32_t  g_wsPendingStation = 0;
uint32_t g_wsPendingSinceMs = 0;

void queueWsStation(int32_t station) {
  portENTER_CRITICAL(&g_wsPlayMux);
  g_wsPendingStation = station;
  g_wsPendingSinceMs = millis();
  portEXIT_CRITICAL(&g_wsPlayMux);
}

void processPendingWsStation() {
  if (!player.readyForWebStation()) {
    return;
  }
  ...
  portENTER_CRITICAL(&g_wsPlayMux);
  if (g_wsPendingStation > 0 &&
      static_cast<uint32_t>(now - g_wsPendingSinceMs) >= WS_PLAY_SETTLE_MS) {
    station = g_wsPendingStation;
    g_wsPendingStation = 0;
  }
  portEXIT_CRITICAL(&g_wsPlayMux);
  ...
  if (stationCount == 0) {
    log_w("##[WSPLAY]# ignored: playlist is empty");
    return;
  }
  ...
  if (!player.sendCommandNoWait({PR_PLAY, station})) {
    // Keep the newest selection pending if the player queue is temporarily full.
    queueWsStation(station);
    log_w("##[WSPLAY]# player queue full, retry station=%ld", static_cast<long>(station));
    return;
  }
...
//DLNA mod
  ...
  // Download DLNA playlist → PC-re mentés (attachment)
  webserver.on("/dlna/playlist_dlna.csv", HTTP_GET, [](AsyncWebServerRequest *request) {
    if (!LittleFS.exists(PLAYLIST_DLNA_PATH)) {
      request->send(404, "text/plain", "No DLNA playlist built yet");
      return;
    }
    AsyncWebServerResponse *r = request->beginResponse(LittleFS, PLAYLIST_DLNA_PATH, "text/csv");
    r->addHeader("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0");
    r->addHeader("Pragma", "no-cache");
    r->addHeader("Content-Disposition", "attachment; filename=\"playlist_dlna.csv\"");
    request->send(r);
  });

  // Upload DLNA playlist → felülírja a playlist_dlna.csv-t (TMP_PATH-on keresztül)
  webserver.on(
    "/dlna/playlist_dlna.csv", HTTP_POST,
    [](AsyncWebServerRequest *request) {
      if (!request->_tempFile) {
        request->send(400, "application/json", "{\"ok\":false,\"msg\":\"no file received\"}");
        return;
      }
      ...
      if (!LittleFS.rename(TMP_PATH, PLAYLIST_DLNA_PATH)) {
  ...
  // ── Volume Curve endpoints ──────────────────────────────────────────────
  webserver.on("/volcurve.html", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response = request->beginResponse(LittleFS, "/www/volcurve.html.gz", "text/html");
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0");
    request->send(response);
  });
  ...
  webserver.on("/volcurve.csv", HTTP_GET, [](AsyncWebServerRequest *request) {
    String csv = config.volumeCurveToCsv();
  ...
  webserver.on("/volcurve.csv", HTTP_POST,
    [](AsyncWebServerRequest *request) {
      if (!request->_tempFile) {
        request->send(400, "application/json", "{\"error\":\"csv body missing\"}");
        return;
      ...
      if (config.applyVolumeCurveCsv(csv.c_str(), &importError)) {
        if (config.saveVolumeCurveToFile()) {
          request->send(200, "application/json", "{\"ok\":true}");
        } else {
          request->send(500, "application/json", "{\"error\":\"cannot persist volume curve\"}");
        }
      } else {
        if (!importError.length()) { importError = "invalid volume curve csv"; }
        importError.replace("\\", "\\\\");
        importError.replace("\"", "\\\"");
        request->send(400, "application/json", ("{\"error\":\"" + importError + "\"}").c_str());
      }
    },
    nullptr,
    [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
  ...
  webserver.on("/volcurve.csv", HTTP_DELETE, [](AsyncWebServerRequest *request) {
    config.setDefaultVolumeCurve();
    if (!config.saveVolumeCurveToFile()) {
      request->send(500, "application/json", "{\"error\":\"cannot persist default volume curve\"}");
      return;
    }
    float lut[22];
    lut[0] = -60.0f;
    for (int i = 1; i <= 21; ++i) { lut[i] = (float)config.store.volumeCurveDb[i - 1]; }
...
void NetServer::chunkedHtmlPage(const String &contentType, AsyncWebServerRequest *request, const char *path) {
  ...
  // browsers to keep serving a stale cached playlist.csv on export even
  // after the file was updated on the device, making it look like edits
  // weren't being saved.
  response->addHeader("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0");
  request->send(response);
}
...
void NetServer::processQueue() {
    ...
    switch (request.type) {
      ...
      case GETSYSTEM:
      {
        ...
        int wsPos = snprintf(
          wsBuf,
          sizeof(wsBuf),
          "{\"sst\":%d,\"aif\":%d,\"rssiastext\":%d,\"vu\":%d,\"vupeak\":%d,\"vubox\":%d,\"vumidpct\":%d,\"vuhighpct\":%d,\"softr\":%d,\"mdns\":\"%s\",\"ipaddr\":\"%s\", \"watchdog\": %d, \"stallwatchdog\": %d, \"seriallittlefs\": %d, "
          "\"nameday\": %d, \"clocktts\": %d, \"clockttslang\": \"%.2s\", \"clockttsinterval\": %u, ",
          config.store.smartstart != 2, config.store.audioinfo, config.store.rssiAsText, config.store.vumeter, config.store.vuPeakOn, config.store.vuMidOn,
          config.store.vuMidPctDef, config.store.vuHighPctDef,
          config.store.softapdelay, config.store.mdnsname,
        ...
        if (wsPos > 0 && static_cast<size_t>(wsPos) < sizeof(wsBuf)) {
          snprintf(
            ...
            "\"lsenabled\": %d, \"lsssena\": %d, \"lsmodel\": %d, \"lsbright\": %d, \"lscount\": %d, \"vustyle\": %d, \"aeq\": %d }",
            config.store.clockTtsOnlyWhenNoStream,
            config.store.clockTtsQuietHoursEnabled,
            quietFrom,
            quietTo,
            config.store.lsEnabled, config.store.lsSsEnabled,
            config.store.lsModel, config.store.lsBrightness, config.store.lsCount,
            config.store.vuSpecMode,
            config.store.autoEqEnabled
          );
        }
        break;
      }
  ...
  sprintf(
    ...
    "\"scrpe\":%d,\"scrpt\":%d,\"scrpb\":%d,\"scrst\":%d,"
  ...
  break;
      ...
      case GETCONTROLS:
      {
        ...
        sprintf(
          wsBuf, "{\"irtl\":%d,\"encind\":%d,\"encindavail\":%d,\"skipup\":%d,\"autoStartTime\":\"%s\",\"autoStopTime\":\"%s\"}",
          config.store.irtlp,
          dualEncodersConfigured ? config.store.encodersIndependent : 0, dualEncodersConfigured, config.store.skipPlaylistUpDown,
          config.store.autoStartTime[0] ? config.store.autoStartTime : "",
          config.store.autoStopTime[0]  ? config.store.autoStopTime  : ""
        );
        break;
      }
      ...
      {  //DLNA mod
        ...
        const char* btAvailable = "true";
#else
        const char* btAvailable = "false";
#endif
        snprintf(wsBuf, sizeof(wsBuf), "{\"playermode\": \"%s\", \"btavailable\": %s}", playerMode, btAvailable);
        break;
      }
      ...
      case GETVOLCURVE: {
        ...
        for (int i = 1; i <= 21; ++i) {
          ...
          json += String(i);
          json += "\",\"value\":";
          json += String((int)config.store.volumeCurveDb[i - 1]);
          json += '}';
        }
        json += "]}";
        if (clientId == 0) {
          websocket.textAll(json.c_str());
        } else {
          websocket.text(clientId, json.c_str());
        }
        return;
      }
    }
...
void NetServer::onWsMessage(void *arg, uint8_t *data, size_t len, uint8_t clientId) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    char wsCmd[65];
    char wsVal[65];
    if (len >= sizeof(wsLine)) {
      log_w("##[WS]# command too long: %u bytes", static_cast<unsigned>(len));
      return;
    }
    memcpy(wsLine, data, len);
    wsLine[len] = '\0';

    if (config.parseWsCommand(wsLine, wsCmd, wsVal, sizeof(wsCmd))) {
      if (strcmp(wsCmd, "ping") == 0) {
        websocket.text(clientId, "{\"pong\": 1}");
        return;
      }
      if (strcmp(wsCmd, "playstation") == 0 || strcmp(wsCmd, "play") == 0) {
        int32_t station = atoi(wsVal);
        if (station < 1) {
          station = 1;
        }
        queueWsStation(station);
      ...
      if (strcmp(wsCmd, "playlist") == 0 && strcmp(wsVal, "web") == 0) {
      ...
      if (strcmp(wsCmd, "playlist") == 0 && strcmp(wsVal, "dlna") == 0) {
      ...
      /* ----- Auto On-Off Timer ----- */
      if (strcmp(wsCmd, "autoStartTime") == 0) {
        strlcpy(config.store.autoStartTime, wsVal, sizeof(config.store.autoStartTime));
        config.eepromWrite(EEPROM_START, config.store);
        return;
      }
      if (strcmp(wsCmd, "autoStopTime") == 0) {
        strlcpy(config.store.autoStopTime, wsVal, sizeof(config.store.autoStopTime));
        config.eepromWrite(EEPROM_START, config.store);
        return;
      }
      if (strcmp(wsCmd, "reset") == 0 && strcmp(wsVal, "timer") == 0) {
      ...
      if (cmd.exec(wsCmd, wsVal, clientId)) {
        return;
      }
    }
  }
